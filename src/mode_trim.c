 ...
 */
{
  ...
  cur_pos = 0;
  cur_max = min(info->data_size - (uint32_t)cur_total,XFER_SIZE);
...
static void scan_file(wave_info *info,uint32_t *skip_beginning,uint32_t *skip_end,progress_info *proginfo)
{
  int sample_size,i;
  bool is_silence,found_noise;
  char sample[BUF_SIZE];
  uint32_t bytes_to_read,bytes_remaining,tmp_beginning_bytes,tmp_end_bytes;
...
static bool trim_file(wave_info *info)
{
  ...
  uint32_t skip_beginning = 0,skip_end = 0,data_bytes = 0;
