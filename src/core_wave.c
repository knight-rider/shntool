...
/* determines whether the given filename (info->filename) is a regular file, and is readable */
{
  ...
  info->actual_size = (uint32_t)sz.st_size;
...
bool do_header_kluges(uint8_t *header,wave_info *info)
{
  bool retval = TRUE;
...
{
  uint32_t le_long=0, bytes;
  uint8_t buf[2];
  uint8_t tag[4];
  int header_len = 0;

  /* look for "RIFF" in header */
  if (!read_tag(info->input,tag) || tagcmp(tag,(uint8_t *)WAVE_RIFF)) {
    if (verbose) {
      if (!tagcmp(tag,(uint8_t *)AIFF_FORM)) {
  ...
  if (!read_tag(info->input,tag) || tagcmp(tag,(uint8_t *)WAVE_WAVE)) {
  ...
  for (;;) {
    ...
    if (!tagcmp(tag,(uint8_t *)WAVE_FMT))
      break;
  ...
  for (;;) {
    ...
    if (!tagcmp(tag,(uint8_t *)WAVE_DATA))
      break;
  ...
  info->rate = (info->samples_per_sec * info->channels * info->bits_per_sample) / 8;
...
*/
  ...
  if (info->data_size > info->total_size - (uint32_t)info->header_size)
...
void make_canonical_header(uint8_t *header,wave_info *info)
/* constructs a canonical WAVE header from the values in the wave_info struct */
{
  if (NULL == header)
    return;

  tagcpy(header,(uint8_t *)WAVE_RIFF);
  ulong_to_uchar_le(header+4,info->chunk_size);
  tagcpy(header+8,(uint8_t *)WAVE_WAVE);
  tagcpy(header+12,(uint8_t *)WAVE_FMT);
  ...
  tagcpy(header+36,(uint8_t *)WAVE_DATA);
  ulong_to_uchar_le(header+40,info->data_size);
}

char *format_to_str(uint16_t format)
{
  switch (format) {
...
void put_chunk_size(uint8_t *header,uint32_t new_chunk_size)
...
void put_data_size(uint8_t *header,int header_size,uint32_t new_data_size)
