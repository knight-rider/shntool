...
static bool input_header_kluge(uint8_t *,wave_info *);
...
static bool parse_aiff_header(char *filename, uint32_t *samples, uint16_t *channels,uint16_t *bits_per_sample)
/* generic function to parse an AIFF header and store certain values contained therein */
{
  ...
  *bits_per_sample = 0;
  ...
  if (!read_tag(f,tag) || tagcmp(tag,(uint8_t *)AIFF_FORM))
  ...
  if (tagcmp(tag,(uint8_t *)AIFF_FORM_TYPE_AIFF) && tagcmp(tag,(uint8_t *)AIFF_FORM_TYPE_AIFC))
    goto invalid_aiff_header;

  if (!tagcmp(tag,(uint8_t *)AIFF_FORM_TYPE_AIFC))
    is_compressed = TRUE;
  ...
  while (1) {
    ...
    if (!tagcmp(tag,(uint8_t *)AIFF_COMM))
      break;
  ...
  if (is_compressed) {
    ...
    if (tagcmp(tag,(uint8_t *)AIFF_COMPRESSION_NONE) && tagcmp(tag,(uint8_t *)AIFF_COMPRESSION_SOWT)) {
...
static bool input_header_kluge(uint8_t *header,wave_info *info)
