...
static void adjust_for_leadinout(uint32_t leadin_bytes,uint32_t leadout_bytes)
{
  int current,pass;
  uint32_t this_leadin,this_leadout;
...
static bool split_file(wave_info *info)
{
  ...
  int current;
  uint32_t discard,bytes;
  bool success;
  uint32_t leadin_bytes, leadout_bytes, bytes_to_xfer;
  ...
  for (current=0;current<numfiles;current++) {
    ...
    files[current]->length = files[current]->data_size / (uint32_t)info->rate;
...
{
  uint32_t repeated_split_size = smrt_parse((unsigned char *)repeated_split_point,info);
  uint32_t bytes_left;
