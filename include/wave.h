...
typedef struct _wave_info {
	...
	uint32_t id3v2_tag_size;	/* size of the ID3v2 tag this file contains, if any	*/
} wave_info;
...
void make_canonical_header(uint8_t *buf,wave_info *info);

/* returns a string corresponding to the WAVE format code given */
char *format_to_str(uint16_t);
...
bool do_header_kluges(uint8_t *,wave_info *);
